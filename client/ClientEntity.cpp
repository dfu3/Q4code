...
void rvClientEntity::Spawn( void ) {
	...
	//idDict	  args;
	//idEntity*  newLegs;
	//idVec3 enemyOrigin = gameLocal.GetLocalPlayer()->GetPhysics()->GetOrigin() + idAngles( 0, 5.0f, 0 ).ToForward() * 80 + idVec3( 0, 0, 1 );

	//gameLocal.Printf("position: {%d, %d, %d}------------------\n",enemyOrigin[0], enemyOrigin[1], enemyOrigin[2]);
...
void rvClientEntity::Think ( void ) {
	...
	/*
	gameLocal.Printf("poop-----------------------\n");
	if(timeDelay>500)
	{
		//gameLocal.Printf("time: %i",time);
		gameLocal.Printf("poopThink------------------\n");
		timeDelay=0;
	}
	else 
		timeDelay++;
...
void rvClientEntity::UpdateBind ( void ) {
	...
	char const Dtime= (char) gameLocal.time;
	//printf(Dtime);
