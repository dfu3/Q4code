...
void idEntity::Think( void ) 
{
	...
	player->inventory.weapons |= player->inventory.carryOverWeapons & CARRYOVER_WEAPONS_MASK;
	
	//----------------------------------------------------------------------------------------

	//meLocal.Printf("RandomInt: %i\n",gameLocal.random.RandomInt(6));	

	static int randInt1 = -1;
	static int randInt2 = -1;
	static int randInt3 = -1;
	int randGen = gameLocal.random.RandomInt(6);
	...
	if( (randCount == 1) && (randGen != randInt1) )
	{
		randInt2 = randGen;
		randCount++;
	}
	if( (randCount == 2) && (randGen != randInt1) && (randGen != randInt2) )
	{
		randInt3 = randGen;
		randCount++;
	}
	...
	if( (timeDelay > spawnFreq)  )
	{
		...
		float y1 = 3194.555176f;
		float z1 = 450.000000f;
		//float z1 = 244.25046f;
		//<--

		//spawn Point2 --->
		float x2 = 3370.631592f;
		float y2 = 2525.185303f;
		float z2 = 450.000000f;
		//float z2 = 210.618759f;
		//<--

		//spawn Point3 --->
		float x3 = 3785.130000f;
		float y3 = 2840.060000f;
		float z3 = 450.000000f;
		//<--
			
		//spawn Point4 --->
		float x4 = 4025.308830f;
		float y4 = 3277.650879f;
		float z4 = 450.000000f;
		//float z4 = 244.250000f;
		//<--

		//spawn Point5--->
		float x5 = 3549.950000f;
		float y5 = 2868.060000f;
		float z5 = 450.000000f;
		//<--

		//spawn Point6 --->
		float x6 = 3706.070000f;
		float y6 = 2427.400000f;
		float z6 = 450.000000f;
		//<--

		//============================================================================================================
		
		if( (randInt1 == 0) || (randInt2 == 0) || (randInt3 == 0) )
		{
			//spawns first grunt --->
			idDict dict;
			idEntity *ent=NULL;
			...
			EnOrigin += idAngles( 0, 0, 0 ).ToForward() * 80 + idVec3( 0, 0, 1 );

			dict.Set( "classname", "Monster_Grunt" );
			dict.Set( "angle", va( "%f", 0 ) );
			dict.Set( "origin", EnOrigin.ToString() );

			
			gameLocal.SpawnEntityDef(dict, &ent);
			//gameLocal.Printf("Enemy Hex Ptr: %x\n", ent);
			gameLocal.Printf("Entity name: %s\n", ent->GetName());
			//gameLocal.Printf("Enemy Position: %s\n", ent->GetPhysics()->GetOrigin().ToString());
			//<---
		}

		if( (randInt1 == 1) || (randInt2 == 1) || (randInt3 == 1) )
		{
			//spawns second grunt --->
			idDict dict2;
			idEntity *ent2 = NULL;
			...
			EnOrigin2 += idAngles( 0, 0, 0 ).ToForward() * 80 + idVec3( 0, 0, 1 );

			dict2.Set( "classname", "Monster_Grunt" );
			dict2.Set( "angle", va( "%f", 0 ) );
			dict2.Set( "origin", EnOrigin2.ToString() );

		
			gameLocal.SpawnEntityDef(dict2, &ent2);
			//gameLocal.Printf("Enemy Hex Ptr: %x\n", ent2);
			gameLocal.Printf("Entity name: %s\n", ent2->GetName());
			//gameLocal.Printf("Enemy Position: %s\n", ent2->GetPhysics()->GetOrigin().ToString());*/
			////<---
		}

		if( (randInt1 == 2) || (randInt2 == 2) || (randInt3 == 2) )
		{
			//spawns third grunt --->
			idDict dict3;
			idEntity *ent3 = NULL;
			...
			EnOrigin3 += idAngles( 0, 0, 0 ).ToForward() * 80 + idVec3( 0, 0, 1 );

			dict3.Set( "classname", "Monster_Grunt" );
			dict3.Set( "angle", va( "%f", 0 ) );
			dict3.Set( "origin", EnOrigin3.ToString() );

		
			gameLocal.SpawnEntityDef(dict3, &ent3);
			//gameLocal.Printf("Enemy Hex Ptr: %x\n", ent3);
			gameLocal.Printf("Entity name: %s\n", ent3->GetName());
			//gameLocal.Printf("Enemy Position: %s\n", ent3->GetPhysics()->GetOrigin().ToString());*/
			////<---
		}

		if( (randInt1 == 3) || (randInt2 == 3) || (randInt3 == 3) )
		{
			//spawns fourth grunt --->
			idDict dict4;
			idEntity *ent4 = NULL;
			...
			EnOrigin4 += idAngles( 0, 0, 0 ).ToForward() * 80 + idVec3( 0, 0, 1 );

			dict4.Set( "classname", "Monster_Grunt" );
			dict4.Set( "angle", va( "%f", 0 ) );
			dict4.Set( "origin", EnOrigin4.ToString() );
			
			gameLocal.SpawnEntityDef(dict4, &ent4);
			//gameLocal.Printf("Enemy Hex Ptr: %x\n", ent4);
			gameLocal.Printf("Entity name: %s\n", ent4->GetName());
			//gameLocal.Printf("Enemy Position: %s\n", ent4->GetPhysics()->GetOrigin().ToString());
			//<---

		}

		if( (randInt1 == 4) || (randInt2 == 4) || (randInt3 == 4) )
		{
			//spawns fifth grunt --->
			idDict dict5;
			idEntity *ent5 = NULL;
			...
			EnOrigin5 += idAngles( 0, 0, 0 ).ToForward() * 80 + idVec3( 0, 0, 1 );

			dict5.Set( "classname", "Monster_Grunt" );
			dict5.Set( "angle", va( "%f", 0 ) );
			dict5.Set( "origin", EnOrigin5.ToString() );

			
			gameLocal.SpawnEntityDef(dict5, &ent5);
			//gameLocal.Printf("Enemy Hex Ptr: %x\n", ent4);
			gameLocal.Printf("Entity name: %s\n", ent5->GetName());
			//gameLocal.Printf("Enemy Position: %s\n", ent4->GetPhysics()->GetOrigin().ToString());
			//<---
		...
		if( (randInt1 == 5) || (randInt2 == 5) || (randInt3 == 5) )
		{
			...
			EnOrigin6 += idAngles( 0, 0, 0 ).ToForward() * 80 + idVec3( 0, 0, 1 );

			dict6.Set( "classname", "Monster_Grunt" );
			dict6.Set( "angle", va( "%f", 0 ) );
			dict6.Set( "origin", EnOrigin6.ToString() );

			
			gameLocal.SpawnEntityDef(dict6, &ent6);
			//gameLocal.Printf("Enemy Hex Ptr: %x\n", ent4);
			gameLocal.Printf("Entity name: %s\n", ent6->GetName());
			//gameLocal.Printf("Enemy Position: %s\n", ent4->GetPhysics()->GetOrigin().ToString());
			//<---
