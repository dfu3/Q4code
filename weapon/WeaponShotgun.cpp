...
stateResult_t rvWeaponShotgun::State_Idle( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_WAIT:			
			...
			if (!clipSize ) {
				if ((gameLocal.score > 60) && gameLocal.time > nextAttackTime && wsfl.attack && AmmoAvailable ( ) ) {
					SetState( "Fire", 0 );
					return SRESULT_DONE;
				}  
			} else {				
				if ((gameLocal.score > 60) &&  gameLocal.time > nextAttackTime && wsfl.attack && AmmoInClip ( ) ) {
					SetState( "Fire", 0 );
					return SRESULT_DONE;
				}  
...
stateResult_t rvWeaponShotgun::State_Fire( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_WAIT:
			...
			if ((gameLocal.score > 60) && wsfl.attack && gameLocal.time >= nextAttackTime && AmmoInClip() ) {
				SetState( "Fire", 0 );
				return SRESULT_DONE;
			}
			if ( clipSize ) {
