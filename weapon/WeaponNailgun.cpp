...
stateResult_t rvWeaponNailgun::State_Idle( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_WAIT:
			...
			if ( !clipSize ) {
				if ((gameLocal.score > 100) &&  gameLocal.time > nextAttackTime && wsfl.attack && AmmoAvailable ( ) ) {
					SetState ( "Fire", 0 );
					return SRESULT_DONE;
				}
			} else {
				if ((gameLocal.score > 100) &&  gameLocal.time > nextAttackTime && wsfl.attack && AmmoInClip ( ) ) {
					SetState ( "Fire", 0 );
					return SRESULT_DONE;
				}  
...
stateResult_t rvWeaponNailgun::State_Fire( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			...
			if ((gameLocal.score > 100) &&   DrumSpin ( NAILGUN_DRUMSPEED_FAST, 2 ) ) {
				PostState ( "Fire", 2 );
				return SRESULT_DONE;
			}
...
stateResult_t rvWeaponNailgun::State_DrumSpinDown ( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_WAIT:
			...
			if ((gameLocal.score > 100) &&   wsfl.attack && AmmoInClip ( ) ) {
				viewModel->StopSound ( NAILGUN_SPIN_SNDCHANNEL, false );
				SetState ( "Fire", 4 );
				return SRESULT_DONE;
			}
