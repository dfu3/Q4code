...
stateResult_t rvWeaponGrenadeLauncher::State_Idle( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_WAIT:			
			...
			if (!clipSize ) {   // only fire if score is > 10
				if ( (gameLocal.score > 40) &&  wsfl.attack && AmmoAvailable ( ) ) {
					SetState ( "Fire", 0 );
					return SRESULT_DONE;
				}
			} else { 
				if ( (gameLocal.score > 40) &&  gameLocal.time > nextAttackTime && wsfl.attack && AmmoInClip ( ) ) {
					SetState ( "Fire", 0 );
					return SRESULT_DONE;
				}  
...
stateResult_t rvWeaponGrenadeLauncher::State_Fire ( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_WAIT:		
			if ( (gameLocal.score > 40) &&  wsfl.attack && gameLocal.time >= nextAttackTime && AmmoInClip() && !wsfl.lowerWeapon ) {
				SetState ( "Fire", 0 );
				return SRESULT_DONE;
			}
