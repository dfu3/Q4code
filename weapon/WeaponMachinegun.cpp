...
stateResult_t rvWeaponMachinegun::State_Idle( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_WAIT:			
			...
			if ( !clipSize ) {
				if ( (gameLocal.score > 20) && !fireHeld && gameLocal.time > nextAttackTime && wsfl.attack && AmmoAvailable ( ) ) {
					SetState ( "Fire", 0 );
					return SRESULT_DONE;
				}
			} else {
				if ( (gameLocal.score > 20) && !fireHeld && gameLocal.time > nextAttackTime && wsfl.attack && AmmoInClip ( ) ) {
					SetState ( "Fire", 0 );
					return SRESULT_DONE;
				}  
